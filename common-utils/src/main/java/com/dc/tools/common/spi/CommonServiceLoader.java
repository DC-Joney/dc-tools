...
import com.dc.tools.common.utils.Requires;
 ...
 */
public final class CommonServiceLoader<S> implements Iterable<S> {
    ...
    private CommonServiceLoader(final Class<S> service, final ClassLoader loader) {
        this.service = Requires.requireNonNull(service, "service interface cannot be null");
        this.loader = (loader == null) ? ClassLoader.getSystemClassLoader() : loader;
        reload();
    }

    public static <S> CommonServiceLoader<S> load(final Class<S> service) {
        return CommonServiceLoader.load(service, Thread.currentThread().getContextClassLoader());
    }

    public static <S> CommonServiceLoader<S> load(final Class<S> service, final ClassLoader loader) {
        return new CommonServiceLoader<>(service, loader);
    }
    ...
    public Iterator<S> iterator() {
        return new Iterator<S>() {
            ...
            final Iterator<Map.Entry<String, S>> knownProviders = CommonServiceLoader.this.providers.entrySet()
                    .iterator();

            @Override
            public boolean hasNext() {
                return this.knownProviders.hasNext() || CommonServiceLoader.this.lookupIterator.hasNext();
            }
            ...
            public S next() {
                ...
                final Class<S> cls = CommonServiceLoader.this.lookupIterator.next();
                return newProvider(cls);
            }
    ...
    public Iterator<Class<S>> classIterator() {
        return new Iterator<Class<S>>() {
            ...
            public Class<S> next() {
                ...
                return CommonServiceLoader.this.lookupIterator.next();
            }
